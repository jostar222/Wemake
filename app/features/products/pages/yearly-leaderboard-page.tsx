...
export const meta: Route.MetaFunction = ({params}) => {
  const date = DateTime.fromObject({
    ...
    .setZone("Asia/Seoul")
    .setLocale("ko-KR");
  return [{title: `Best of year of ${date.toLocaleString({
          year: "numeric",
        })} | wemake`}];
}
...
export default function YearlyLeaderboardPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-10">
      <Hero 
        title={`Best of year of ${urlDate.toLocaleString({
          year: "numeric",
        })}`}
      />
      <div className="flex items-center justify-center gap-2">
        <Button variant="secondary" asChild>
          <Link 
            to={`/products/leaderboards/yearly/${previousYear.year}`} // 그냥 year 사용 시 해가 걸치는 경우에 잘못된 날짜로 이동하여 year 사용
          >
            &larr; 
