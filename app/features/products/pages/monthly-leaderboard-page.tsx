...
export const meta: Route.MetaFunction = ({params}) => {
  const date = DateTime.fromObject({
    ...
    .setZone("Asia/Seoul")
    .setLocale("ko-KR");
  return [{title: `Best of month of ${date.toLocaleString({
          month: "long",
          year: "2-digit",
        })} | wemake`}];
}
...
export default function MonthlyLeaderboardPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div className="space-y-10">
      <Hero 
        title={`Best of month of ${urlDate.toLocaleString({
          month: "long",
          year: "2-digit",
        })}`}
      />
      <div className="flex items-center justify-center gap-2">
        <Button variant="secondary" asChild>
          <Link 
            to={`/products/leaderboards/monthly/${previousMonth.year}/${previousMonth.month}`} // 그냥 month 사용 시 해가 걸치는 경우에 잘못된 날짜로 이동하여 month 사용
          >
            &larr; 
